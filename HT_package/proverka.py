...
print(list_15)
#from_list_15 = list_15.slice(2, 4)
#print('from_list', from_list_15)
my_list = [1, 2, 3, 4, 5, 6]
first_part, middle_part, last_part = my_list.split(2, 4)
...
for row in blank_karta:
    poz_in_lin = sorted(random.sample(range(1, 10),
                                      5))  # генератор позиц (неповтор) в линии карты (range +1 не включая последний элемент)
    ...
    print(' '.join(
        str(elem) for elem in row))  # Вывод на экран карточки, используя генератора строки из списка через Join
